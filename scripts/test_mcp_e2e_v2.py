import os, sys, asyncio, json
# Clear inherited PYTHONPATH (points at Hermes venv → pydantic_core conflict)
if "PYTHONPATH" in os.environ:
    del os.environ["PYTHONPATH"]
sys.path.insert(0, r"C:\AI\MT5_MCP\MT5-MCP-main\src")
from mcp_mt5.main import mcp

def parse(r):
    """Parse a FastMCP call_tool result (TextContent wrapper)."""
    txt = r.text if hasattr(r, "text") else str(r)
    if txt.startswith("content=[TextContent") or txt.startswith("content=["):
        # Extract the JSON/text payload
        import re
        m = re.search(r"text='([^']*)'", txt, re.DOTALL)
        txt = m.group(1) if m else txt
    try:
        return json.loads(txt)
    except Exception:
        return {"raw": txt[:300]}

async def run(name, **params):
    try:
        r = await mcp.call_tool(name, arguments=params)
        return parse(r)
    except Exception as e:
        return {"error": f"{type(e).__name__}: {str(e)[:200]}"}

async def main():
    print("=== initialize (portable, MT5 running, PYTHONPATH cleared) ===")
    i = await run("initialize")
    print("init:", i)

    print("=== get_account_info ===")
    a = await run("get_account_info")
    if "login" in a:
        print("login:", a.get("login"), "server:", a.get("server"), "balance:", a.get("balance"), "trade_allowed:", a.get("trade_allowed"))
    else:
        print(a)

    print("=== list_strategies ===")
    s = await run("list_strategies")
    n = s.get("strategies") or (len(s) if isinstance(s, list) else 0)
    print("strategies:", n, "items")

    print("=== compile_mql5 ===")
    c = await run("compile_mql5", **{"filepath": r"C:\AI\MT5_ICMarkets_Global\MQL5\Experts\Examples\Moving Average\Moving Average.mq5"})
    print("compile:", {k: c.get(k) for k in ("success", "ex5_exists")})

    print("=== shutdown ===")
    print(await run("shutdown"))

asyncio.run(main())
